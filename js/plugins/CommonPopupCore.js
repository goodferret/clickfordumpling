//
//  汎用ポップアップコア ver1.04 | 通用彈出核心 此為自用的翻譯版本
//
//  author yana
//
...
/*:
 * @plugindesc ver1.04/提供通用彈出機制的基本插件。
 * 汎用的なポップアップの仕組みを提供するためのベースプラグインです。
 * @author Yana
 * 
 * @param Text Back Color
 * @desc ポップアップの背景カラーです。彈出窗口的背景顏色。
 * rgba(red,green,blue,alpha)で設定してください。
 * @default rgba(0,0,0,0.6)
 * 
 * @param Text Back FileName
 * @desc ポップアップの背景画像名です。彈出窗口的背景圖像名稱。
 * %dがインデックスに変換されます。%d 被轉換為索引。
 ...
 * sx:表示位置補正X
 * sy:表示位置補正Y
 * pattern:顯示模式 0 為淡入淡出，-1 為水平拉伸，-2 為垂直拉伸
 * back:-1：透明背景，0：填充背景色，1或以上：圖像索引
 * bx:内容の表示位置補正X
 * by:内容の表示位置補正Y
 * extend:在顯示時序調整數組中指定。 
   例：extend:[20,50] 出現超過 20 幀並在第 50 幀開始消失。
 * fixed:画面に固定するか？ true/falseで指定。
 * anchorX:
 * anchorY:
 * slideCount:當出現新的彈出窗口時向上滑動的速度有多快。
 * 
 * ------------------------------------------------------
 * 利用規約：特になし。素材利用は自己責任でお願いします。
 * ------------------------------------------------------
 * 該插件是一個提供通用彈出機制的插件。
  * 這個插件除了添加插件命令之外沒有其他功能。
  * 作為附加功能，
  * ・\I[x] 繪製的圖標尺寸遵循字符尺寸。
  * ・添加了改變字符大小的控製字符\FS[x]。
  * 增加2分。
 ...
 * 在運動模式中添加了垂直拉伸和水平拉伸。
 * 修復了彈出窗口的插件參數無法正常工作的錯誤。
 * ver1.03:
 * 更改為可以在後面的參數中指定圖片文件夾的文件名。
 * ver1.02:
 * 增加了使幻燈片從上到下工作的功能。
 * ver1.01:
 * 修復了在不創建容器的情況下可以註冊彈出窗口的錯誤。
 * ver1.00:
 * 公開
 */
