...
/*:
 * @plugindesc 動態字符串圖片生成器插件
 * @author トリアコンタン
 *
 * @param itemIconSwitchId
 * @text 項目圖標開關 ID
 * @desc 當指定編號的開關打開時，圖標顯示為\ITEM[n]。 如果不指定，則始終顯示。
 * @default 0
 * @type switch
 *
 * @param lineSpacingVariableId
 * @text 行距修正變量ID
 * @desc 顯示多行時的行間距由指定變量的值修正。 如果設置的值太大，可能會被截斷。
 * @default 0
 * @type variable
 *
 * @param frameWindowSkin
 * @text 框架視窗皮膚
 * @desc 框架窗口的外觀文件的名稱。 如果您使用窗口生成器，則必須指定。
 * @default
 * @require 1
 * @dir img/system/
 * @type file
 ...
 * @param padCharacter
 * @text 填充文字
 * @desc 這是在繪製數值時未達到指定位數時填充的字符。 請僅指定一個單字節字符。
 * @default 0
 *
 * @param prefixText
 * @text 前綴字符串
 * @desc 在每個字符串圖片之前插入文本。 主要指定默認控製字符。
 * @default
 *
 * @help 提供使用指定字符串動態生成圖片的命令。
 * 字符串中還可以使用各種控製字符（\v[n]等），用控製字符顯示的變量值為
 * 當圖片內容發生變化時，您可以實時更新。
 *
 * 以下の手順で表示します。
 *  1 :使用插件命令 [D_TEXT] 指定要繪製的字符串和參數（請參見下面的示例）
 *  2 : 使用插件命令 [D_TEXT_SETTING] 指定背景顏色和對齊方式（可選）
 *  3 : 在事件命令“顯示圖片”中將“圖像”指定為未選中狀態。
 * ※ 在步驟1時圖片並不會顯示，所以可以等指令都輸入好再作為一個集合來調用。
 * ※ 在顯示圖片之前多次調用D_TEXT可以顯示多行。
 *
 * 從ver1.4.0開始，如果執行[D_TEXT]後在“顯示圖片”中指定“圖像”，
 * 則暫停動態字符串圖片生成，並照常顯示“圖像”圖片。
 *
 * プラグインコマンド詳細
 ...
 *
 * 如果用了WINDOW視窗來顯示圖片, 可以再設定光柵(在標題畫面會出現的那個在選項背景閃爍的東西的東西)
 * 要在DTEXT顯示出圖片之後再設定
