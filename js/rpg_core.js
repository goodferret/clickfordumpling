...
//-----------------------------------------------------------------------------
/**
 * 表示圖像的基本對象。The basic object that represents an image.
 *
 * @class Bitmap
 * @constructor
...
//對於 iOS。 img 消耗內存。 所以重複使用它。for iOS. img consumes memory. so reuse it.
Bitmap._reuseImages = [];


/**
 * 位圖狀態Bitmap states(Bitmap._loadingState):
 *
 * none:
 * Empty Bitmap
 *
 * 待辦的pending:
 * URL 已請求，但等待加載，直到調用 
 * startRequestUrl requested, but pending to load until startRequest called
 *
 * 清除的purged:
